...
| Commander | CLI | `setwin --help`, `status`, `init`, identity, twin, Gherkin, workflow, and review commands | Node stdlib argv parsing is too low-level | MIT | Required | Another CLI toolkit in `@setwin/cli` |
...
| tsx | TypeScript execution | Run CLI/API during Phase 0 without a separate emit step | `tsc` would also work | MIT | Development | `tsc` + `node` |
| TypeScript | Language | Plan of record is TypeScript | — | Apache-2.0 | Required | — |

PostgreSQL 16 is the primary database and is run locally from `docker-compose.yml`.
